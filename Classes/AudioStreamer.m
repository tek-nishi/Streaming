...
NSString * const AS_NETWORK_CONFIG_FAILED_STRING = @"Unable to configure network read stream.";
NSString * const AS_STREAM_CREATE_FAILED_STRING = @"Unable to create stream.";
NSString * const AS_STREAM_OPEN_FAILED_STRING = @"Unable to open stream.";
...
+ (NSString *)stringForErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	switch (anErrorCode)
	{
		...
		case AS_NETWORK_CONFIG_FAILED:
			return AS_NETWORK_CONFIG_FAILED_STRING;
		case AS_STREAM_CREATE_FAILED:
			return AS_STREAM_CREATE_FAILED_STRING;
		case AS_STREAM_OPEN_FAILED:
			return AS_STREAM_OPEN_FAILED_STRING;
		default:
			return AS_AUDIO_STREAMER_FAILED_STRING;
	}
...
//
-(void)showAlertView:(AudioStreamerErrorCode)anErrorCode wait:(BOOL)aWait
{
#ifdef TARGET_OS_IPHONE			
	UIAlertView *alert =
		[[[UIAlertView alloc]
			 initWithTitle:NSLocalizedStringFromTable(@"Audio Error", @"Errors", nil)
			 message:NSLocalizedStringFromTable([AudioStreamer stringForErrorCode:anErrorCode], @"Errors", nil)
			 delegate:self
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		[self showAlertView:anErrorCode wait:NO];
	}
}
...
- (BOOL)openFileStream
{
	...
	{
		NSAssert(stream == nil,
			@"CFReadStreamRef already initialized");
		
#ifndef USE_HTTP_STREAMING
		stream = CFReadStreamCreateWithFile(NULL, (CFURLRef)url);
		if(!stream)
		{
			[self showAlertView:AS_STREAM_CREATE_FAILED wait:YES];
		}
#else
		//
		...
		{
			[self showAlertView:AS_NETWORK_CONFIG_FAILED wait:YES];
			return NO;
		}
		...
		{
			CFRelease(stream);
			[self showAlertView:AS_STREAM_OPEN_FAILED wait:YES];
			return NO;
		}
...
- (void)startInternal
{
	...
	@synchronized(self)
	{
		...
		if (![self openAudioStream] || ![self openFileStream])
		{
			goto cleanup;
		}
	}
...
playback:
	...
	if(loopStreaming && stopReason == AS_STOPPING_EOF)
	{
		@synchronized(self)
		{
			// FIXME:閉じて開くのが勿体ない
			...
			// FIXME:ファイルがAACだと、これも閉じて開かにゃならん！
...
- (double)progress
{
	@synchronized(self)
	{
		if (sampleRate > 0 && ![self isFinishing])
		{
			...
			double progress = queueTime.mSampleTime / sampleRate;
			if (progress < 0.0)
			{
				progress = 0.0;
			}
